(() => {
  ...
  async function renderMarkdown(rawMd, sourceLabel='') {
    try {
      ...
      // save math globally for diagnostics / late repairs
      window.__MDV_MATH_ARRAY__ = math.slice();
      window.__MDV_MATH_LOOKUP__ = {};
      math.forEach((it, i) => { window.__MDV_MATH_LOOKUP__[String(i)] = it; });
  ...
  window.addEventListener('drop', async e => {
    ...
    const f = e.dataTransfer && e.dataTransfer.files and e.dataTransfer.files[0];
